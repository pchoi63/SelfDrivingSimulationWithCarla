...
class World(object):
    ...
    def getLightState(self):
        if self.player.is_at_traffic_light():
            traffic_light = self.player.get_traffic_light()
            answer = traffic_light.get_state()
            #print(str(answer) + " - Ground Truth")
            return answer

    def speedLimit(self):
        print(self.get_speed_limit())

    def currSpeed(self):
        global currSpeed
        v = self.player.get_velocity()
        currSpeed = (3.6 * math.sqrt(v.x ** 2 + v.y ** 2 + v.z ** 2))
        return currSpeed
    ...
    def parse_events(self, client, world, clock):
        ...
        if not self._autopilot_enabled:
            if isinstance(self._control, carla.VehicleControl):
                ...
                diff = targetSpeed - currSpeed
                #print("Target Speed" + str(targetSpeed))
                #print("Current Speed" + str(cur))
                #print("Difference" + str(diff))
                global lightState
                if lightState == "Red":
                    self._control.brake = 1.0

                elif lightState == "Orange":
                    self._control.brake = 1.0
                '''
                else:
                    if diff > 0:
                        thrStr = diff / 20
                        self._control.throttle = thrStr
                self._control.steer = test_con
                '''
...
class CameraManager(object):
    ...
    def set_sensor(self, index, notify=True):
        ...
        if needs_respawn:
            ...
            # circular reference.
            weak_self = weakref.ref(self)
            self.sensor.listen(lambda image: CameraManager._parse_image(weak_self, image, nett))
        if notify:
    ...
    @staticmethod
    def _parse_image(weak_self, image, net):
        self = weak_self()
        if not self:
            return
        if self.sensors[self.index][0].startswith('sensor.lidar'):
            ...
            global test_con
            global currSpeed
            prediction, newIm = yoloPredict(size_im, net)
            test_con = laneDetect(roi_im, newIm, targetSpeed, currSpeed, lightState)

            if prediction >= 6:
                speedChange(prediction)
            elif prediction >= 3:
                lightChange(prediction)
